...
class ICF(InstanceSelectionMixin):
    ...
    def ennpadrao(self, X, y):
        ...
        for i in range(X.shape[0]):
            ...
            if classifier.predict(temp) != [y[i]]:
                mask[i] = not mask[i]
    ...
    def select_data(self, X, y):
        ...
        rechable, coverage = np.zeros(y.size), np.zeros(y.size)
    ...
    #     """Zwraca stan serializowalny."""
    #     state = self.__dict__.copy()
    #     state.pop('pairwise_distances', None)  # Usuń obiekt nieserializowalny
    #     return state

    # def __setstate__(self, state):
    #     """Przywraca stan obiektu."""
    #     self.__dict__.update(state)
    #     self.pairwise_distances = None  # Przywróć do wartości domyślnej
